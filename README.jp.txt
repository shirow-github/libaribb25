...
【更新履歴】
  ・2024,11/20 - ver. 0.2.10
    ...
    macOS向けパッチを取り込んだ。
    autoconf 2.71 にて廃止されたマクロを置き換えた。

  ・2020,12/15 - ver. 0.2.9
	
    libpcsclitebcasを使用するように変更しました。
    TSストリームの変更への自動追従に失敗する事があるのを修正しました。
    アダプテーションフィールドのみのパケットのスクランブルフラグの処理を修正しました。
    put()がエラーになった場合の処理の変更と、未処理バッファの回収APIの追加しました。
  ...
  ・2016,02/18 - ver. 0.2.7
    ...
    ドキュメントを更新し、OSX でのコンパイルを修正するためのマイナーリリース。

  ・2014,10/21 - ver. 0.2.6

    ビルドシステムをautoconfに移行しました。
    パケットサイズを設定して、検出プロセスをスキップできるようにします。
    さまざまな修正とプラットフォーム互換性の修正。
